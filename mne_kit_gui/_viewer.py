...
def _sph_to_cart_view(azimuth, elevation, distance, focalpoint):
    ...
    view_up = (0., 0., 1.) if 5. <= abs(elevation) <= 175. else (0., 1., 0.)
...
class HeadViewController(HasTraits):
    ...
    def on_set_view(self, view, _=''):
        ...
        position, view_up = _sph_to_cart_view(az, el, distance, (0., 0., 0.))
        self.scene.camera_position = [
            tuple(position), (0., 0., 0.), view_up]
...
class PointObject(Object):
    ...
    def _show_labels(self, change):
        ...
        if show and self.scene is not None and len(self.points) > 0:
            ...
            self.scene.add_point_labels(
                pts, labels, text_color=self.color, shape=None,
                show_points=False, font_size=int(self.label_scale * 1000),
                name=name, always_visible=True)
    ...
    def _project(self):
        ...
        if (nearest is None or len(nearest.data) <= 1 or len(pts) == 0 or
                self.project_to_trans is None):
        ...
        if self.mark_inside and not self.project_to_surface:
            scalars = (~self.check_inside(proj_rr, verbose=False)).astype(
                float)
    ...
    def _plot_points(self):
        ...
        if self._view == 'arrow':
            ...
            height = DEFAULTS['coreg']['eegp_height'] \
                if mode == 'cylinder' else None
            geom = _glyph_geom(mode, resolution=self.resolution,
                               solid_transform=transform, height=height)
        ...
        if use_scalars:
            ...
            kwargs.update(scalars='mark_inside', cmap=ListedColormap(
                [self.inside_color, self.color]), clim=[0., 1.],
                show_scalar_bar=False)
        else:
            kwargs['color'] = self.color
        self.glyph = self.scene.add_mesh(mesh, name=self.name or None,
                                         render=False, **kwargs)
    ...
    @observe('points', 'project_to_trans', 'project_to_surface',
             'mark_inside', 'nearest', 'orient_to_surface', 'resolution',
             'scale_by_distance')
    ...
    def plot(self):
        ...
        if self._block_behind:
            self.surf_rear = self.scene.add_mesh(
                mesh, color=self.color, style=style, line_width=1,
                opacity=self.rear_opacity, culling='front', pickable=False,
                render=False)
            self.surf_rear.SetVisibility(self.visible)

        self.surf = self.scene.add_mesh(
            mesh, color=self.color, style=style, line_width=1,
            opacity=self.opacity, culling='back', pickable=True,
            render=False)
    ...
    def _points_changed(self, change):
        ...
        if self._update_points() and self.src is not None:
            self.src.compute_normals(
                ...
                split_vertices=False, consistent_normals=False,
                non_manifold_traversal=False, inplace=True)
            if self.scene is not None:
                self.scene.render()
